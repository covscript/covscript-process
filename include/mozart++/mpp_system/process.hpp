...
namespace mpp_impl {
    ...
    void set_cloexec(int fd)
	{
		if (fcntl(fd, F_SETFD, fcntl(fd, F_GETFD) | FD_CLOEXEC) == -1)
		    mpp::throw_ex<mpp::runtime_error>("Error setting FD_CLOEXEC flag.");
	}
    ...
    void create_process_unix(const process_startup &startup,
        ...
        if (pid < 0) {
            ...
            if (!startup._stdin.redirected()) {
                close_fd(pstdin[PIPE_READ]);
                set_cloexec(pstdin[PIPE_WRITE]);
            }
            if (!startup._stdout.redirected()) {
                close_fd(pstdout[PIPE_WRITE]);
                set_cloexec(pstdout[PIPE_READ]);
            }
...
namespace mpp {
    ...
    private:
        struct _member_holder
        {
            ...
            fdistream _stderr;
            int _exit_code = -1;
            explicit _member_holder(const process_info& info) : _info(info), _stdin(_info._stdin), _stdout(_info._stdout), _stderr(_info._stdout) {}
            ~_member_holder(){
